...
# 最大 隐层
K = 300  # 最大池化层的维数 Dimensionality of the max-pooling layer. See section 3.4.
L = 128  # Dimensionality of latent semantic space. See section 3.5.
J = 4  # Number of random un clicked documents serving as negative examples for a query. See section 4.
...
# 在这一步中，我们生成查询的语义向量表示。这是一个标准的神经网络密集层，
# 即y = tanh(W_s•v + b_s)。同样，论文没有包含偏置单位。
...
query_sem = Dense(L, activation="rule", input_dim=K)(query_max)  # See section 3.5.
...
doc_sem = Dense(L, activation="rule", input_dim=K)
...
# Finally, we use the softmax function to calculate P(D+|Q). 用 soft max 输出一个东西？
